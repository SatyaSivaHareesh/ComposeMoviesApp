...
class MainViewModelTest {
    ...
    fun setUp() {
        ...
        viewModel = MainViewModel(getPopularMoviesUseCase)
    }
    ...
    fun `getPopularMoviesUseCase emits success`() = runTest {
        ...
        assertThat(currentState.value).isInstanceOf(MainViewModel.MainViewState.Success::class.java)
    }

    @Test
    fun `getPopularMoviesUseCase emits success with empty list`() = runTest {
        whenever(getPopularMoviesUseCase.getPopularMovies(any())).thenAnswer {
            flow { emit(Resource.Success(data = arrayListOf<Movie>())) }
        }
        viewModel.getMovies(page)
        val currentState = viewModel.getViewState()
        assertThat(currentState.value).isInstanceOf(MainViewModel.MainViewState.SuccessWithEmptyData::class.java)
    }
    ...
    fun `getPopularMoviesUseCase emits error`() = runTest {
        ...
        viewModel.getMovies(page)
        val currentState = viewModel.getViewState()
        assertThat(currentState.value).isInstanceOf(MainViewModel.MainViewState.Error::class.java)
    }
    ...
    fun `getPopularMoviesUseCase emits loading`() = runTest {
        ...
        viewModel.getMovies(page)
        val currentState = viewModel.getViewState()
        assertThat(currentState.value).isInstanceOf(MainViewModel.MainViewState.Loading::class.java)
    }
    ...
    fun `verify setLoading function called with isLoading=true `() = runTest {
        ...
        val currentState = viewModel.getViewState()
        assertThat(currentState.value).isInstanceOf(MainViewModel.MainViewState.Loading::class.java)
        val loadingState = currentState.value as MainViewModel.MainViewState.Loading
